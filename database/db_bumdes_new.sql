...
CREATE TABLE `pelunasan_pinjaman` (
  ...
  `notif` enum('-','Sudah Baca','Belum Baca') NOT NULL DEFAULT '-'
...
CREATE TABLE `permohonan_pinjaman` (
  `id_permohonan` int(11) NOT NULL,
  `id_peminjam` int(11) NOT NULL,
  `peminjaman_ke` int(11) NOT NULL,
  `jumlah_pinjam` double NOT NULL,
  `durasi_angsuran` int(11) NOT NULL,
  `tanggal` date NOT NULL,
  `tanggal_persetujuan` date DEFAULT NULL,
  `status` enum('Sudah Disetujui','Tidak Disetujui','Belum Disetujui') NOT NULL DEFAULT 'Belum Disetujui'
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
--
-- Constraints for table `pelunasan_pinjaman`
--
ALTER TABLE `pelunasan_pinjaman`
  ADD CONSTRAINT `pelunasan_pinjaman_ibfk_1` FOREIGN KEY (`id_permohonan`) REFERENCES `permohonan_pinjaman` (`id_permohonan`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `permohonan_pinjaman`
--
ALTER TABLE `permohonan_pinjaman`
  ADD CONSTRAINT `permohonan_pinjaman_ibfk_1` FOREIGN KEY (`id_peminjam`) REFERENCES `peminjam` (`id_peminjam`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
